%%==============================================================================%%
%%                  BATCH TRACKING GUI FOR DSI STUDIO: ROI COMBINATIONS					%%
%%==============================================================================%%

%  
% roi_combinations.m
% 
% This GUI is designed for batch tracking in DSI studio. In this verison, the user selects ROI
% files, and tracking is performed between all possible combinations of selected files.
%
...
function outputdirectory_button_Callback(hObject, eventdata, handles)
  ...
  % Open a file selection dialog to get path of desired output directory, limit selectable items to folders only
	directory  = uigetdir('*.*','Select location for output file');
...
function add_ROI_button_Callback(hObject, eventdata, handles)
  ...
  if ~isequal(selected_files, 0);
    ...
    if iscell(selected_files);
      % Yes, so add all the files to the file list
    	file_list = cat(1, file_list, selected_files(:));
  	else
      % They only selected one file, so only add that one to the file list
  		file_list = cat(1, file_list, selected_files);
  	end
  end
...
% NOTE: See primary_roi.m or roi_pairs.m for code comments for this function.
...
% NOTE: See primary_roi.m or roi_pairs.m for code comments.
